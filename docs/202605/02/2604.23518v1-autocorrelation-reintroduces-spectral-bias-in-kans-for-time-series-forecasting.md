---
title: Autocorrelation Reintroduces Spectral Bias in KANs for Time Series Forecasting
title_zh: 自相关在时间序列预测中重新引入了 KAN 的谱偏置
authors: "Chen Zeng, Jiahui Wang, Qiao Wang"
date: 2026-04-26
pdf: "https://arxiv.org/pdf/2604.23518v1"
tags: ["query:urt-pf"]
score: 7.0
evidence: 具有时间自相关的序列预测
tldr: 本研究探讨了Kolmogorov-Arnold网络（KANs）在时间序列预测中的表现。尽管理论上KANs能克服神经网络常见的谱偏差，但研究发现时间序列的强自相关性会重新引入该偏差，且偏差随自相关程度增加而加剧。为此，研究者提出利用离散余弦变换（DCT）进行预处理以降低输入相关性，有效缓解了低频偏好问题，提升了预测性能。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2604.23518v1/fig-001.webp\", \"caption\": \"TABLE I: Experimental settings for the synthetic spectral-bias experiments.\", \"page\": 8, \"index\": 1, \"width\": 1076, \"height\": 315}, {\"url\": \"assets/figures/arxiv/2604.23518v1/fig-002.webp\", \"caption\": \"Fig. 2: Autocorrelation-wise performance of KAN and DCT-KAN.\", \"page\": 10, \"index\": 2, \"width\": 1093, \"height\": 555}]"
motivation: 探究在时间序列预测中，输入变量的强自相关性是否会破坏KANs克服谱偏差的理论优势。
method: 通过理论分析和实证验证自相关性对谱偏差的影响，并引入离散余弦变换（DCT）对输入数据进行去相关预处理。
result: 实验证明时间序列的自相关性确实会导致KANs出现明显的谱偏差，而DCT预处理能显著减少网络对低频成分的偏好。
conclusion: 标准KANs在处理强自相关的时间序列时存在局限性，通过去相关技术可以有效恢复其捕捉高频信息的能力。
---

## 摘要
现有理论表明，在输入统计独立的假设下，Kolmogorov-Arnold 网络 (KAN) 可以克服神经网络中常见的谱偏置。然而，这一假设在时间序列预测 (TSF) 中并不成立，因为其输入是具有强时间自相关的滞后观测值。通过理论分析和实证验证，我们得到了一个出人意料的发现：时间自相关在 KAN 中重新引入了谱偏置，且随着自相关程度的增加，这种偏置变得愈发显著。这表明标准 KAN 在处理具有强自相关输入的 TSF 任务时可能会面临巨大困难。为解决这一问题，我们引入了离散余弦变换 (DCT) 以降低网络输入之间的相关性。正如预期的那样，实验结果表明 DCT 预处理显著减少了 TSF 中观察到的低频偏好。这一结果也证实了 KAN 在 TSF 任务中的谱偏置确实是由输入变量之间的自相关引起的。

## Abstract
Existing theory suggests that Kolmogorov-Arnold Networks (KANs) can overcome the spectral bias commonly observed in neural networks under the assumption that inputs are statistically independent. However, this assumption does not hold in time series forecasting (TSF), where inputs are lagged observations with strong temporal autocorrelation. Through theoretical analysis and empirical validation, we obtain an unexpected finding: temporal autocorrelation reintroduces spectral bias in KANs, and the bias becomes increasingly pronounced as the degree of autocorrelation increases. This suggests that standard KANs may face substantial difficulties in TSF with strongly autocorrelated inputs. To address this problem, we introduce the Discrete Cosine Transform (DCT) to reduce the correlations among the network inputs. As expected, experimental results reveal that DCT preprocessing substantially reduces the observed low-frequency preference in TSF. This result also corroborates that the spectral bias of KANs in TSF tasks is indeed induced by the autocorrelation among input variables.