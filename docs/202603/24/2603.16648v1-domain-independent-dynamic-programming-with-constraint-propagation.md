...
## 摘要
组合问题存在两种流行的基于模型的范式：1) 基于状态的表示，如启发式搜索、动态规划 (DP) 和决策图；2) 基于约束和领域的表示，如约束规划 (CP)、(混合) 整数规划和布尔可满足性。在本文中，我们通过将约束传播集成到 DP 中，弥合了 DP 和 CP 范式之间的差距，使 DP 求解器能够利用约束传播来剪枝状态和转换。为此，我们在领域无关动态规划框架中使用通用 CP 求解器实现了约束传播，并在三个组合优化问题上使用启发式搜索进行了评估：带时间窗的单机调度问题、资源受限项目调度问题 (RCPSP) 以及带时间窗的旅行商问题 (TSPTW)。我们的评估表明，约束传播显著减少了状态扩展的数量，使得我们的方法在单机调度和 RCPSP 问题上比 DP 求解器解决了更多的实例，并在高度受限的 TSPTW 实例中表现出类似的改进。运行性能表明，对于受限实例，传播带来的收益超过了其开销，但进一步减少传播开销的研究工作可以进一步提高性能。我们的工作是理解 DP 求解器中约束传播价值的关键一步，为集成 DP 和 CP 提供了一种基于模型的方法。
