...
## 摘要
智能交通系统日益依赖来自路侧摄像头、无人机影像、激光雷达和车载传感器的异构数据，然而，这些数据源之间缺乏统一的数据标准、模型接口和评估协议，阻碍了研究结果的可复现性、跨数据集基准测试以及跨区域的可迁移性。现有的轨迹数据集在坐标系、对象表示和元数据字段方面遵循不兼容的规范，迫使研究人员为每个数据集和模拟器的组合构建自定义的预处理流水线。为了应对这些挑战，我们提出了 Ozone，这是一个围绕五个互连层（硬件、数据、模型、评估和原型）组织的统一交通研究平台，每一层都具有标准化的模式、自动转换流水线和互操作接口。在首个版本中，该数据模式将四个轨迹数据集（NGSIM、highD、CitySim 和 UTE）统一为一种规范格式，包含定向边界框、运动学变量和预计算的代理安全指标。CARLA 中的数字孪生地图和校准后的交通模型提供了集成的基准测试环境。在人因研究、交通场景生成和安全关键建模方面的案例研究表明，Ozone 将实验设置时间缩短了 85%，使安全模型的跨城市迁移效率达到 91%，并将跨数据集的可复现性提高到 3% 的方差以内。源代码和数据集已公开。
