...
tldr: 针对动态时空图联邦学习（FSTG）中存在的空间和时间异质性问题，本文提出了一种受因果启发的框架 SC-FSGL。该框架通过表示层面的干预，将可迁移的因果知识与客户端特定的噪声解耦。通过引入条件分离模块和因果代码簿，SC-FSGL 能够有效缓解表示纠缠和负迁移，在五个异质性时空图数据集上均取得了优于现有方法的效果。
source: arxiv
...
motivation: 现有联邦图学习方法忽略了时空图的异质性，导致虚假表示纠缠和负迁移，影响了模型的泛化性能。
method: 提出 SC-FSGL 框架，利用条件分离模块解耦因果因子，并通过因果代码簿进行原型聚类与对比学习以促进跨客户端知识共享。
result: 在五个具有不同异质性的时空图数据集上的实验表明，SC-FSGL 的性能显著优于现有的最先进方法。
conclusion: 通过因果干预显式解耦不变特征与特定噪声，是提升异质动态时空图联邦学习性能的有效途径。
---
