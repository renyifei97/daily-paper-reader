...
motivation: 传统的社交导航方法往往仅对当前观测做出反应，缺乏对机器人动作如何影响未来场景和行人运动的预见性推理。
method: 提出一种基于动作条件的世界模型，利用深度特征进行自回归预测，并通过强化学习结合未来特征注入和社交奖励塑造来优化导航策略。
result: 在 Social-HM3D 等基准测试中取得了最先进的导航成功率，并成功在 Unitree Go2 机器人上完成了零样本迁移的实机部署。
conclusion: NavThinker 证明了将动作条件下的未来预测与规划相结合，能显著提升机器人在复杂动态社交环境中的安全性和导航效率。
---

## 摘要
社交导航要求机器人在动态的人类环境中安全行动。有效的行为需要前瞻性思考：推理场景和行人在不同机器人动作下如何演变，而非仅仅对当前观测做出反应。这产生了一个耦合的预测-规划挑战，即机器人动作与人类运动相互影响。为应对这一挑战，我们提出了 NavThinker，这是一个将动作条件世界模型与同策略强化学习（on-policy reinforcement learning）相结合的未来感知框架。该世界模型在 Depth Anything V2 的 patch 特征空间中运行，并对未来场景几何和人类运动进行自回归预测；随后，多头解码器生成未来深度图和人类轨迹，从而产生与可通行性和交互风险相一致的未来感知状态。关键在于，我们使用 DD-PPO 训练策略，同时通过以下方式注入世界模型的前瞻信号：(i) 将动作条件下的未来特征融合到当前观测嵌入中；(ii) 基于预测的人类轨迹进行社交奖励塑造。在单机器人和多机器人 Social-HM3D 上的实验表明，该方法取得了最先进的导航成功率，并实现了向 Social-MP3D 的零样本迁移以及在 Unitree Go2 上的实机部署，验证了其泛化能力和实际应用价值。网页：https://github.com/hutslib/NavThinker。
