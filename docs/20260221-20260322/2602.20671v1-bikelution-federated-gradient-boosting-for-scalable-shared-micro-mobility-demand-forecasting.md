...
## 摘要
无桩共享单车系统的快速增长产生了海量的时空数据集，这些数据对于车队分配、缓解拥堵和可持续出行具有重要价值。然而，单车需求取决于多种外部因素，使得传统的时间序列模型难以胜任。集中式机器学习 (CML) 虽然能提供高精度的预测，但当数据分布在边缘设备上时，会引发隐私和带宽问题。为了克服这些局限性，我们提出了 Bikelution，这是一种基于梯度提升树的高效联邦学习 (FL) 解决方案，它在保护隐私的同时，能够提供长达六小时的准确中期需求预测。在三个真实共享单车系统 (BSS) 数据集上的实验表明，Bikelution 的表现与基于 CML 的变体相当，且优于当前的最先进技术。研究结果突显了隐私感知需求预测的可行性，并概述了 FL 和 CML 方法之间的权衡。
