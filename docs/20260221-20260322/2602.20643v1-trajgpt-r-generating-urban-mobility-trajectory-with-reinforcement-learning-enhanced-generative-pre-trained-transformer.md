...
tags: ["query:urt"]
score: 6.0
evidence: 强化学习用于城市移动轨迹生成
tldr: 针对城市移动轨迹数据因隐私受限的问题，本文提出TrajGPT-R框架。该框架将轨迹生成视为离线强化学习问题，通过预训练与微调两阶段过程实现。利用逆强化学习从历史数据中推断个体偏好并构建奖励模型，随后对Transformer模型进行微调，有效解决了传统强化学习在长程信用分配和稀疏奖励方面的难题。实验证明该方法在可靠性和多样性上显著优于现有模型，为城市规划提供了强有力的模拟工具。
selection_source: fresh_fetch
motivation: 城市移动轨迹数据对城市规划至关重要，但由于隐私保护限制，获取大规模真实数据面临挑战。
method: 提出一种结合离线强化学习与逆强化学习的Transformer框架，通过两阶段训练优化轨迹生成过程并捕捉个体偏好。
